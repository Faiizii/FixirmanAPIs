...
class Provider extends REST_Controller {
	...
	public function check_phone_number_post(){
		...
		$phoneResponse = $this->Provider_model->check_phone_number($params);
		if($phoneResponse){
	...
	public function get_user_info_post(){
		...
		$userInfo = $this->Provider_model->get_user($params['user_id'],$params['phone'],$params['user_type']);
		if($userInfo){
	...
	public function register_user_post(){
		$params = $this->input->post();
		$this->validate_post(array('name','cnic','phone','user_type','password',
			'categories','login_with','token','serial'));

		$userInfo = $this->Provider_model->register_provider($params);
		if($userInfo){
			$this->response($userInfo, 200);
		}else{
			$this->response($this->response_failed(), 200);
	...
	public function login_post(){
		$params = $this->input->post();
		$this->validate_post(array('phone','user_type','password','token','serial'));

		$userInfo = $this->Provider_model->check_login($params);
		if($userInfo){
			$this->response($userInfo, 200);
		}else{
			$this->response($this->response_failed(), 200);
	...
	public function update_password_post(){
		$params = $this->input->post();
		$this->validate_post(array('phone','user_type','password'));

		$userInfo = $this->Provider_model->update_password($params);
		if($userInfo){
			$this->response($userInfo, 200);
		}else{
			$this->response($this->response_failed(), 200);
	...
	public function update_profile_post(){
		...
		$userInfo = $this->Provider_model->update_profile($params);
		if($userInfo){
			$this->response($userInfo, 200);
		}else{
			$this->response($this->response_failed(), 200);
	...
	public function save_address_post(){
		...
		$isSaved = $this->Provider_model->save_address($params);
	...
	public function delete_address_post(){
		...
		$isDeleted = $this->Provider_model->delete_address($params);
	...
	public function get_faq_post(){
		...
		$faq = $this->Provider_model->get_faq($params);
	...
	public function get_employee_reviews_post(){
		$params = $this->input->post();

		$this->validate_post(array('user_id'));

		$reviews = $this->Provider_model->get_employee_reviews($params);
